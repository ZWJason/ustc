...
def train_kd(model, teacher_model, optimizer, loss_fn_kd, dataloader, metrics, params):
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, (train_batch, labels_batch) in enumerate(dataloader):
            ...
            t.update()
    d = t.format_dict
    process_speed = (d["n"]/d["elapsed"])*params.batch_size
...
def train_and_evaluate_kd(model, teacher_model, train_dataloader, val_dataloader, optimizer,
    ...
    for epoch in range(params.num_epochs):
        ...
        # compute number of batches in one epoch (one full pass over the training set)
        process_speed = train_kd(model, teacher_model, optimizer, loss_fn_kd, train_dataloader,
                 metrics, params)
        speed.append(process_speed)
        # Evaluate for one epoch on validation set
        ...
        #     # board_logger.histo_summary(t ag+'/grad', value.grad.data.cpu().numpy(), epoch+1)
...
# 定义自适应训练
def train_akd(model, teacher_model,assistant_model,optimizer, loss_fn_kd, dataloader, metrics, params,flag,kl_array,kl_flag):
    ...
    with tqdm(total=len(dataloader)) as t:
        for i, (train_batch, labels_batch) in enumerate(dataloader):
            ...
            if kl_array[i] < 0.003 or kl_flag[i] == False:
                with torch.no_grad():
                    output_teacher_batch = teacher_model(train_batch)
                if params.cuda:
                    output_teacher_batch = output_teacher_batch.cuda()
                ...
                with torch.no_grad():
                    output_teacher_batch = assistant_model(train_batch)
                if params.cuda:
                    output_teacher_batch = output_teacher_batch.cuda()
            
            kdl_loss = F.kl_div(F.log_softmax(output_batch, dim=1),F.softmax(output_teacher_batch, dim=1))
            loss = loss_fn_kd(output_batch, labels_batch, output_teacher_batch, params)
            kl_array[i] = kdl_loss

            if flag == True:
                t.write(str(kdl_loss.double()))
                flag = False
            # 计算astant_output_batch和output_batch的相对熵
            #kdl_loss = nn.KLDivLoss()(F.log_softmax(output_batch, dim=1),F.softmax(astant_output_batch, dim=1))
            #kdl_loss = F.kl_div(F.log_softmax(output_batch, dim=1),F.softmax(astant_output_batch, dim=1))
...
# 定义自适应知识蒸馏
def train_and_evaluate_akd(model, teacher_model, assistant_model, train_dataloader, val_dataloader, optimizer,
                       loss_fn_kd, metrics, params, model_dir, restore_file=None):
    """Train the model and evaluate every epoch.
    ...
    # learning rate schedulers for different models:
    if params.model_version == "resnet18_adaptive_distill":
        scheduler = StepLR(optimizer, step_size=150, gamma=0.1)
    # for cnn models, num_epoch is always < 100, so it's intentionally not using scheduler here
    elif params.model_version == "cnn_adaptive_distill": 
        scheduler = StepLR(optimizer, step_size=100, gamma=0.2) 
    
    speed = []
    acc = []
    kl_array = [1 for i in range(len(train_dataloader))]
    kl_flag = [True for i in range(len(train_dataloader))]
    for epoch in range(params.num_epochs):
        optimizer.step()
        #scheduler.step()
        ...
        logging.info("Epoch {}/{}".format(epoch + 1, params.num_epochs))
        flag = True
        process_speed = train_akd(model,teacher_model,assistant_model, optimizer, loss_fn_kd, train_dataloader,
                    metrics, params,flag,kl_array,kl_flag)
        speed.append(process_speed)
        # Evaluate for one epoch on validation set
        val_metrics = evaluate_kd(model, val_dataloader, metrics, params)
...
if __name__ == '__main__':
    ...
    if "adaptive_distill" in params.model_version:
        if params.model_version == "cnn_adaptive_distill":
            model = net.Net(params).cuda() if params.cuda else net.Net(params)
            optimizer = optim.Adam(model.parameters(), lr=params.learning_rate)
            # fetch loss function and metrics definition in model files
            loss_fn_kd = net.loss_fn_kd
            metrics = net.metrics
        elif params.model_version == "resnet18_adaptive_distill":
            model = resnet.ResNet18(numClass).cuda() if params.cuda else resnet.ResNet18()
        ...
        if params.teacher == "resnext29":
            ...
            teacher_model = resnet.ResNet50(numClass)
            teacher_checkpoint = 'experiments/base_resnet50/best.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model
        elif params.teacher == "resnet101":
            teacher_model = resnet.ResNet101(numClass)
            teacher_checkpoint = 'experiments/base_resnet101/best.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model

        if params.assistant_teacher == "resnet18":
            assistant_model = resnet.ResNet18(numClass)
            assistant_checkpoint = 'experiments/base_resnet18/best.pth.tar'
            assistant_model = assistant_model.cuda() if params.cuda else assistant_model
        elif params.assistant_teacher == "resnet50":
            assistant_model = resnet.ResNet50(numClass)
            assistant_checkpoint = 'experiments/base_resnet50/best.pth.tar'
            assistant_model = assistant_model.cuda() if params.cuda else assistant_model


        utils.load_checkpoint(teacher_checkpoint, teacher_model)
        utils.load_checkpoint(assistant_checkpoint, assistant_model)
        ...
        train_and_evaluate_akd(model, teacher_model, assistant_model,train_dl, dev_dl, optimizer, loss_fn_kd,
                              metrics, params, args.model_dir, args.restore_file)

    elif "distill" in params.model_version:
        ...
        if params.teacher == "resnet18":
            teacher_model = resnet.ResNet18(numClass)
            ...
            teacher_model = resnet.ResNet101(numClass)
            teacher_checkpoint = 'experiments/base_resnet101_100/best.pth.tar'
            teacher_model = teacher_model.cuda() if params.cuda else teacher_model
    ...
    # non-KD mode: regular training of the baseline CNN or ResNet-18
        ...
        if params.model_version == "cnn":
            ...
            model = resnet.ResNet50(numClass).cuda() if params.cuda else resnet.ResNet50()
            optimizer = optim.SGD(model.parameters(), lr=params.learning_rate,
                                  momentum=0.9, weight_decay=5e-4)
            # fetch loss function and metrics
            loss_fn = resnet.loss_fn
            ...
            model = resnet.ResNet101(numClass).cuda() if params.cuda else resnet.ResNet101()
            optimizer = optim.SGD(model.parameters(), lr=params.learning_rate,
                                  momentum=0.9, weight_decay=5e-4)
            # fetch loss function and metrics
            loss_fn = resnet.loss_fn
            ...
            model = resnext.CifarResNeXt(cardinality=8, depth=29, num_classes=10).cuda() if params.cuda else resnext.CifarResNeXt(cardinality=8, depth=29, num_classes=10)
            optimizer = optim.SGD(model.parameters(), lr=params.learning_rate,
                                  momentum=0.9, weight_decay=5e-4)
            # fetch loss function and metrics
            loss_fn = resnext.loss_fn
            metrics = resnext.metrics
